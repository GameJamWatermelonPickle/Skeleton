...
Enemy_Up_Spiral::Enemy_Up_Spiral(int x, int y) : Enemy(x, y)
{
	...
	movement.PushBack({ 1.0f,2.0f }, 251, &fly);
	movement.PushBack({ 2.0f,-1.0f }, 180, &fly);
	movement.PushBack({ -1.0f,-2.0f }, 150, &fly);
	movement.PushBack({ -1.0f,2.0f }, 251, &fly);
...
void Enemy_Up_Spiral::Move()
{
	position = originalpos + movement.GetCurrentPosition();
}


void Enemy_Up_Spiral::OnCollision(Collider* c1, Collider* c2) {
