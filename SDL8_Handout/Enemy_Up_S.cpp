...
Enemy_Up_S::Enemy_Up_S(int x, int y) : Enemy(x, y)
{
	...
	redGhostRight.PushBack({ 169,101,47,42 });


	redGhostLeft.PushBack({ 169,153,49,42 });
	redGhostLeft.PushBack({ 169,199,43,43 });
	redGhostLeft.PushBack({ 168,247,43,42 });


	redGhostDown.PushBack({ 274,8,37,42 });
	redGhostDown.PushBack({ 277,54,34,42 });
	redGhostDown.PushBack({ 278,101,33,42 });


	redGhostDown.PushBack({ 225,7,39,42 });
